            ...
            if(keycode === 80) {
                (function() {
                    ...
                    RS.download = function() {
                            var arr = v.tech_.hls.playlists.media_.segments.map(function(x) {return {uri:x.resolvedUri}});

                            console.log('find: ',arr.length,'uris')

                            arr.forEach(window.videojs.Hls.xhr.beforeRequest);
                            ...
                            RS.getfile('tslist', arr.map(function(a) { return a.uri; }).join("\r\n"));
